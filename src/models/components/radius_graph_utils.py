...
def compute_neighbors(pos, num_atoms, edge_index):
    # Get number of neighbors
    # segment_coo assumes sorted index
    ones = edge_index[1].new_ones(1).expand_as(edge_index[1])
    num_neighbors = segment_coo(ones, edge_index[1], dim_size=num_atoms.sum())

    # Get number of neighbors per image
    image_indptr = torch.zeros(num_atoms.shape[0] + 1, device=pos.device, dtype=torch.long)
    image_indptr[1:] = torch.cumsum(num_atoms, dim=0)
...
def radius_graph_pbc(
    atom_pos,  # 3D Cartesian coordinates: (n, 3)
    ...
    """Creates a radial cutoff graph with periodic boundary conditions (PBC).

    Adapted from: https://github.com/FAIR-Chem/fairchem

    Args:
        atom_pos: (n, 3) - 3D Cartesian coordinates
        cell: (bsz, 3, 3) - Lattice vectors/unit cell
        num_atoms: (bsz,) - Number of atoms per crystal
        radius: (float) - Cutoff radius for pairwise distances
        max_num_neighbors_threshold: (int) - Maximum number of neighbors per atom
        enforce_max_neighbors_strictly: (bool) - Enforce strict maximum number of neighbors
        pbc: (list) - Periodic boundary conditions (default: [True, True, True])

    Returns:
        edge_index: (2, e) - Pairs of edges (i, j)
        unit_cell: (e, 3) - Unit cell offsets
        num_neighbors_image: (bsz,) - Number of neighbors per image
    """
    if pbc is None:
        pbc = [True, True, True]
    ...
    # The above line leads to CUDA OOM (tries to allocate 100s of GBs)
    #
    # Older code which worked in DiffCSP repo (TODO check if its from older OCP):
...
def get_max_neighbors_mask(
    ...
    `max_num_neighbors_threshold` neighbors. Assumes that `index` is sorted.

    Enforcing the max strictly can force the arbitrary choice between degenerate edges. This can
    lead to undesired behaviors; for example, bulk formation energies which are not invariant to
    unit cell choice.

    A degeneracy tolerance can help prevent sudden changes in edge existence from small changes in
    atom position, for example, rounding errors, slab relaxation, temperature, etc.
    """
